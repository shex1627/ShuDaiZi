...
class ClaudeWithSources2025:
    ...
    def ask_with_web_search(self, question: str, model: str = None, max_searches: int = 5) -> dict:
        """
        Ask Claude a question with automatic web search using the official Web Search API
        
        Args:
            question: The question to ask
            model: Model to use (defaults to claude-3-5-sonnet, must support web search)
        ...
        # Web search is available on Claude 3.7 Sonnet and Claude 3.5 Sonnet latest
        ...
        try:
            ...
            # Process response content and extract web search citations
            if response.content:
                full_text = ""
                all_citations = []
                        ...
                        if hasattr(content_block, 'citations') and content_block.citations:
                            for citation in content_block.citations:
                                citation_info = {
                                    ...
                                    'page_age': getattr(citation, 'page_age', '')
                                }
            ...
            return {
                ...
                'question': question,
                'model_used': model_to_use,
                'note': 'Web search requires paid Claude account and supported models (Claude 3.7 Sonnet or Claude 3.5 Sonnet latest)'
            }
...
def demonstrate_usage():
    ...
    # Example 1: Web search with the official Web Search API
    question1 = "What are the latest developments in renewable energy in 2025?"
    
    print(f"Question: {question1}")
    print("Using Claude's official Web Search API...")
    
    result1 = claude_client.ask_with_web_search(question1, max_searches=3)
    ...
    # Example 2: Document citations with the Citations API
    documents = [
        {
    ...
    question2 = "What is climate change and what international agreements address it?"
    
    print(f"Question: {question2}")
    print("Using Claude Citations API with documents...")
    
    result2 = claude_client.ask_with_document_citations(question2, documents)
    
    if 'error' in result2:
        print(f"Error: {result2['error']}")
    else:
        print("Formatted Response:")
        print(claude_client.format_citations_response(result2))
        
        print(f"Token usage: {result2['usage']['input_tokens']} input, {result2['usage']['output_tokens']} output")
    
    print("\n" + "="*60 + "\n")
    
    # Example 3: Manual sources (traditional approach)
    manual_sources = [
        {
    ...
    question3 = "What are the current trends in global warming and renewable energy?"
    
    print(f"Question: {question3}")
    print("Using manual sources (traditional approach)...")
    
    result3 = claude_client.ask_with_manual_sources(question3, manual_sources)
    ...
    if 'error' in result3:
        ...
        print(f"Token usage: {result3['usage']['input_tokens']} input, {result3['usage']['output_tokens']} output")
    
    print("\n" + "="*60 + "\n")
    
    # Example 4: PDF citations (requires PDF files)
...
def installation_guide():
    ...
    print("   - Available on Anthropic API, Google Cloud Vertex AI, and AWS Bedrock")
    print("   - Web search requires paid Claude account (feature preview)")
    print("   - Web search available in US for paid users, expanding soon")
    print()
    
    print("9. Advanced Usage:")
    ...
    print("   - Combine web search with document citations for comprehensive answers")
